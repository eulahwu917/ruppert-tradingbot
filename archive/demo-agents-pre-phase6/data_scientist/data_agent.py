...
from agents.data_scientist.logger import log_activity, send_telegram

LOGS_DIR = _PROJECT_ROOT / 'logs'
LOGS_DIR.mkdir(exist_ok=True)

STATE_FILE = LOGS_DIR / 'data_audit_state.json'
PNL_CACHE_FILE = LOGS_DIR / 'pnl_cache.json'
...
def _get_trade_files(since_date: str = '2026-03-26') -> list[Path]:
    ...
    files = []
    for p in sorted(LOGS_DIR.glob('trades_*.jsonl')):
        try:
...
def _today_trades_path() -> Path:
    return LOGS_DIR / f'trades_{date.today().isoformat()}.jsonl'
