...
# P2-1 fix: correct path resolution comments and ensure both paths added.
_DEMO_ROOT = Path(__file__).parent           # environments/demo/
_WORKSPACE_ROOT = _DEMO_ROOT.parent.parent   # workspace/ (was mislabeled as workspace/agents)
# workspace/ must be in sys.path for agents.ruppert.* imports
if str(_WORKSPACE_ROOT) not in sys.path:
    sys.path.insert(0, str(_WORKSPACE_ROOT))
# environments/demo/ must be in sys.path for 'import config' shim
...
def evaluate_crypto_15m_entry(
    ...
    try:
        ...
        position_tracker.add_position(ticker, fill_contracts, direction, fill_price,
                                      module='crypto_15m', title=opp.get('title', ''))
    except Exception as _pt_err:
        logger.warning('[15M-CRYPTO] position_tracker.add_position failed: %s', _pt_err)
