...
def _write_heartbeat(status: str = 'connected') -> None:
    """Write a heartbeat JSON file every 60 s during active WS connection."""
    global _last_heartbeat
    now = time.time()
    if status != 'connected' or now - _last_heartbeat > 60:
        try:
            from agents.ruppert.env_config import get_paths
            hb_path = get_paths()['logs'] / 'ws_heartbeat.json'
...
async def handle_message(msg: dict):
    ...
    _write_heartbeat()  # throttled to once per 60 s
    ...
    # Route crypto hourly band tickers
        ...
        if not any(ticker_upper.startswith(s) for s in CRYPTO_15M_SERIES):
            if yes_ask and yes_bid:
                try:
                    from position_monitor import evaluate_crypto_entry
