...
Optimizer / Researcher → CEO → David (if CEO unsure or real money)
```

- SA-1 (Optimizer), SA-2 (Researcher) → report directly to CEO
- SA-3 (Developer) → output ALWAYS goes to QA before CEO sees it
- SA-4 (QA) → reports PASS/FAIL/WARNINGS; never modifies code; sends FAIL back to Developer with specific issues
...
- CEO escalates to David when:
  ...
  - Core algo parameters change by more than a minor margin
